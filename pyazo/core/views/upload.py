...
class ObjectView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["views"] = context["upload"].objectview_set.order_by("-viewee_date")[
            :10
        ]
        context["forms"] = {
            "collection": CollectionSelectForm(
                prefix="collection",
                initial={"collection": upload.collection},
                data=self.request.POST
                if any("collection" in k for k in self.request.POST.keys())
                else None,
            )
        }
        # Prepare Collections
...
class ClaimObjectView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["delete_url"] = (
            reverse("upload_claim", kwargs={"file_hash": self.kwargs.get("file_hash")}),
        )
...
class DeleteObjectView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["delete_url"] = reverse(
            "upload_delete", kwargs={"file_hash": self.kwargs.get("file_hash")}
        )
        return context
